  ...
  const fetchJobRequests = async () => {
    try {
      ...
      if (!user) {
        ...
        toast.error('Please log in to view job requests.', {
          toastId: 'job-requests-auth-error',
          position: 'top-right',
          autoClose: 2000,
      ...
      toast.success('Job requests loaded successfully 🚀', {
        toastId: 'job-requests-load-success',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
      ...
      toast.error('❌ Failed to fetch job requests. Please try again.', {
        toastId: 'job-requests-load-error',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
  ...
  const acceptJob = async (jobId) => {
    try {
      ...
      toast.success('Job request accepted successfully 🎉', {
        toastId: 'job-requests-accept-success',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
    } catch (err) {
      toast.error('❌ Failed to accept job request. Please try again.', {
        toastId: 'job-requests-accept-error',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
  ...
  const declineJob = async (jobId) => {
    try {
      ...
      toast.info('Job request rejected ❌', {
        toastId: 'job-requests-decline-success',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
    } catch (err) {
      toast.error('❌ Failed to decline job request. Please try again.', {
        toastId: 'job-requests-decline-error',
        position: 'top-right',
        autoClose: 2000,
        hideProgressBar: true,
      });
                    ...
                    acceptedRequests.map((job, index) => (
                      ...
                      >
                        <div className="space-y-4">
                          ...
                          {job.userId?.city && job.userId?.state && (
                            <div className="mt-4 w-full">
                              <div className="h-48 sm:h-56 md:h-64 lg:h-72 w-full rounded-xl overflow-hidden shadow-lg border border-gray-200 bg-gray-50">
                                <Map city={job.userId.city} state={job.userId.state} />
                              </div>
                            </div>
                          )}
                        ...
                        {job.confirmedAt && (
                          <div className="mt-6">
                            <div className="w-full sm:max-w-md px-4 py-3 bg-gradient-to-r from-emerald-500 to-green-500 text-white rounded-lg shadow-md text-center">
                              <span className="block text-sm font-semibold mb-1">✓ CONFIRMED</span>
